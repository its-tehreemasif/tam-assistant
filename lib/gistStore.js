...
const SESSION_FILENAME = 'tam_session.json';   // Separate file: all Baileys Signal key files
...
// ─── Session persistence (Signal key files) ───────────────────────────────────
// Stores the entire Baileys session_assistant/ folder as a single Gist file.
// Without this, every Render restart wipes Signal keys → Bad MAC → no decryption.

async function readSession() {
    if (!token || !gistId) return null;
    try {
        const res = await axios.get(`https://api.github.com/gists/${gistId}`, { headers });
        const content = res.data.files?.[SESSION_FILENAME]?.content;
        if (content) return JSON.parse(content);
    } catch (e) {
        console.error('[GIST] Session read failed:', e.message);
    }
    return null;
}

async function writeSession(sessionFiles) {
    if (!token || !gistId) return false;
    try {
        await axios.patch(`https://api.github.com/gists/${gistId}`, {
            files: {
                [SESSION_FILENAME]: {
                    content: JSON.stringify(sessionFiles, null, 2)
                }
            }
        }, { headers });
        return true;
    } catch (e) {
        console.error('[GIST] Session write failed:', e.message);
        return false;
    }
}

module.exports = { init, readAll, writeAll, readSession, writeSession, localRead, localWrite };
