...
function initScheduler(sock, ownerJid, getPersistence, getAI) {
    // ─── Daily Summary + Quote — 9:00 AM PKT ────────────────────────────────
    cron.schedule('0 9 * * *', async () => {
        ...
        // Morning quote — sent separately so it stands on its own
        try {
            const ai = getAI();
            const q = await ai.chat('_morning_qod_', 'Give me one short powerful motivational quote with author name. Format: "Quote" — Author. Nothing else.');
            ai.resetConversation('_morning_qod_');
            if (q.success) {
                await sendAndDelete(sock, ownerJid, `✨ *Morning Quote*\n\n${q.message}`, [], 120000);
            }
        } catch {}
    }, {
