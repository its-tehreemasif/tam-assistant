...
function getAudioMessage(msg) {
    // Unwrap deviceSentMessage — primary phone messages arrive wrapped in this envelope
    const m = msg.message?.deviceSentMessage?.message || msg.message;
    if (!m) return null;
...
/**
 * Transcribe a WhatsApp audio/voice message with retry logic
 * @returns {{ success: boolean, text?: string, duration?: number, error?: string }}
 */
async function transcribeVoice(msg, apiKey, retryCount = 0) {
    ...
    try {
        ...
        if (buffer.length === 0) {
            return { success: false, error: 'Audio buffer is empty. The file may be corrupted.' };
        }
        ...
        // Send to Groq Whisper with timeout
        ...
        if (!text) {
            // Retry on empty transcription
            if (retryCount < MAX_RETRIES) {
                console.log(`[VOICE] No text detected, retrying (${retryCount + 1}/${MAX_RETRIES})`);
                return transcribeVoice(msg, apiKey, retryCount + 1);
            }
        ...
        if (retryCount < MAX_RETRIES && (error.code === 'ECONNABORTED' || error.code === 'ETIMEDOUT')) {
            console.log(`[VOICE] Network error, retrying (${retryCount + 1}/${MAX_RETRIES})`);
